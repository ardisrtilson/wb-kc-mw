...
router.put('/reset', function requestHandler(req, res) {
  var temp = "a.redford.tilson@gmail.com"
    getToken()
      .then(response => getByEmail(response, temp)
      .then(response => resetPassword(response.token.data.access_token, response.res.data[0].id)
        ))

    res.send(req.body)
});

router.put('/change', function requestHandler(req, res) {
  var temp = "a.redford.tilson@gmail.com"
    getToken()
      .then(response => getByEmail(response, temp)
...
async function resetPassword(token, id) {
    ...
    let url = `/auth/admin/realms/demo-realm/users/${id}/execute-actions-email`
  
    await axios.put(url, data, config).catch(error => console.log(error))
